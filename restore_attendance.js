...
if (content.includes('calPresentsCount')) {
  console.log("Already processed");
  process.exit(0);
}

// Extract calendarLogicAndModal out of the big template literal
...
content = content.slice(0, returnIdx) + calendarLogicAndModal + "\n  " + content.slice(returnIdx);

const endDivIdx = content.lastIndexOf('</div>\n  );\n}');
content = content.slice(0, endDivIdx) + "\n" + modalRender + "\n    " + content.slice(endDivIdx);

fs.writeFileSync(path, content, 'utf8');
console.log('Restored Attendance.jsx Phase 2 successfully.');
